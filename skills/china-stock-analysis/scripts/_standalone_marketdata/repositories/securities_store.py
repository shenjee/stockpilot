...
- 首次构造且表为空时，自动从随仓库分发的 JSON 导入（``ensure_loaded``）。
"""
...
# Standalone skill installs bundle the JSON next to the scripts tree.
_BUNDLED_JSON = Path(__file__).resolve().parents[2] / "securities_master.json"
...
class SecuritiesStore:
    ...
    def ensure_loaded(self) -> None:
        """表为空时从随仓库分发的 JSON 导入一次；已有数据则跳过。

        若 JSON 缺失，打印告警而不是静默吞掉——否则 securities 表为空时前端
        所有搜索都返回空，用户无从得知是主数据未安装而非真的没匹配。
        """
