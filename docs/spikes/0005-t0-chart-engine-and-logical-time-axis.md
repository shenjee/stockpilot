...
| 项 | 目标 | 实测 | 结论 |
|---|---|---|---|
| JS bundle（gzip） | <50KB | 59.10KB | ⚠️ 未达标 |
...
- **包体大小 59.10KB vs 目标 <50KB**：⚠️ 建议接受为限制。相比 ECharts（~300KB+）和 Plotly（~1MB+），Lightweight Charts 仍是最小选项；如需进一步优化，可考虑升级至 5.x 或按需引入。

### 8.4 推荐

**推荐继续使用 Lightweight Charts 4.x 作为首选，并将 ADR 0005 保持为 Proposed/Continue investigation。**

理由：与 PRD/UI 规格中的逻辑时间轴、宽度驱动满轴、follow/manual 状态、回放截断需求匹配度最高，包体最小，无 correctness 阻塞问题。

后续工作建议：
1. 在真实 MacBook Chrome 环境中完成帧率/CPU/内存/延迟测量
2. 决定包体策略：接受现状、按需引入，或升级至 5.x
3. 创建 Draft PR 关联 Issue #25 和 ADR 0005
4. **不修改 ADR 文件和 ADR 状态**
