...
- 项目如何标记 Phase 2 中临时、保守、项目侧派生的结构结果
...
- `fractals`
- `strokes`
- `segments`，Phase 2 中可使用项目侧保守派生策略，但必须在 `meta.mapping_strategy` 中标明
- `pivot_zones`
- `divergences`，Phase 2 默认保持保守输出，未冻结规则前允许为空并给出 warning
- 多周期观察接口预留
...
Phase 2 的 `candidate_buy_points` 和 `candidate_sell_points` 只表示结构候选点，用于图形观察、复盘和后续信号输入。它们不能单独构成交易指令；真正的信号等级、仓位建议和操作结论应留到 Phase 3 的量化信号系统合成。
...
- `strokes`
  对外统一的笔结果列表
- `segments`
  对外统一的线段结果列表；Phase 2 中若由项目侧保守规则派生，必须在 `meta.mapping_strategy` 中说明
- `pivot_zones`
...
- `structure_alerts`
  对外统一的结构变化提醒列表
- `candidate_buy_points`
  对外统一的结构候选买点列表，不等同于交易买入信号
- `candidate_sell_points`
  对外统一的结构候选卖点列表，不等同于交易卖出信号
- `plot_primitives`
...
- `engine_version`
  当前分析结果对应的底层引擎版本
- `parameters`
  当前分析使用的参数快照，用于复现和比较结果

### 7.1.1 输入契约与数据口径

Phase 2 的结构结果对输入口径敏感，`chantheory` 在调用 `czsc` 前必须记录或约束以下输入信息：

- 标准化 OHLCV K 线序列
- 周期参数，如 `1m`、`5m`、`30m`、`day`、`week`
- `czsc` 分析参数，如 `max_bi_num`、`min_bars`
- 复权方式：前复权、后复权或不复权
- 交易日历假设，以及停牌 / 缺失 K 线处理方式
- 不同结构层级所需的最少 K 线数量
- 分钟级 K 线的聚合规则
- 可选的人工校正规则

当前实现已支持 day bar 标准化、最少 K 线数量提示和参数快照；复权方式、交易日历和分钟级聚合仍应作为 Phase 2 收口前的已知 gaps 记录，不应默认为已解决。
...
- 当前 day bar 路径已验证；复权标识、交易日历、停牌处理和分钟级聚合仍是 Phase 2 收口前需要显式记录的 gaps
...
- 是否升级 `czsc` 版本，以及升级后的兼容性回归策略
- `divergences` 是否继续保持保守空输出，或冻结一版项目级定义
- 是否在 Phase 2 首版就暴露多周期联立结果
- `plot_primitives` 是否需要兼容未来 desktop app 的交互特性
- 哪些结构变化需要进入日报提醒，哪些只保留在调试界面
- 复权方式、交易日历、停牌 / 缺失 K 线和分钟级聚合如何进入正式输入契约
