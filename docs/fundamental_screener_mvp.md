...
数据治理是 Fundamental Screener 的独立基础模块，不属于 Streamlit app、skill 或 CLI。真实数据必须先经过采集、缓存、标准化和质量检查，再由 repository 组装成统一的 `MarketSnapshot`，供 core、CLI、Streamlit 和 skill 消费。
...
| 数据 | 第一版主源 | 说明 |
| --- | --- | --- |
| 行业板块列表 | AkShare 封装的东方财富行业板块 | 第一版固定 `classification_system = "em_industry"` |
| 行业板块成分 | AkShare 封装的东方财富行业成分 | 用于生成板块-股票关系 |
| 板块历史行情 | AkShare 封装的东方财富板块行情 | 用于计算板块收益、相对收益和走势图 |
| 个股行情/K线 | 现有腾讯财经 provider | 继续只做行情和技术数据补充 |
| 公司日度快照 | AkShare 东方财富实时行情，必要时腾讯补充 | 市值、PE、PB、PS、换手率、成交额等 |
| 财务指标 | AkShare 公开源 | ROE、毛利率、净利率、营收同比、净利同比、现金流质量、资产负债率等 |
| 新闻/公告/研报 | 暂不接入 | 文本解析复杂，放到后续阶段 |
| Tushare | 暂不考虑 | 需要 token/充值，不作为免费 MVP 主路径 |

腾讯财经已有能力适合继续作为股票实时行情、日 K、分钟 K 和技术指标底层数据源，不应扩展成基本面 provider。财务三表、估值、板块成分和行业口径应进入独立数据治理模块。
...
暂不混用概念板块、申万、中信或自定义板块。原因是行业板块数量较少、重叠少、口径更稳定，更适合验证真实数据链路。概念板块数量通常是行业板块的数倍，且成分重叠严重，后续可作为第二个 `classification_system` 扩展。

### 15.3 本地缓存与表结构

真实数据不应由 Streamlit 或 CLI 每次实时联网获取。第一版应先同步到本地 SQLite，再由 repository 读取。
...
采集层必须把外部字段统一成 core 约定的单位和语义：

- 百分比统一为小数，例如 `0.18` 表示 18%。
- 金额统一为明确单位，优先使用元。
- 日期统一为 `YYYY-MM-DD`。
- 股票代码和市场字段必须能稳定对齐。
- 财务、估值缺失时输出 warnings，不静默吞掉。
- 估值分位优先基于本地历史 PE/PB 自行计算，不直接依赖外部返回。
...
每次生成 `MarketSnapshot` 必须产生 `snapshot_id`，并记录 `analysis_date`、`data_cutoff`、`source_set`、`fetch_run_id`、`quality_report_id`、`config_version`、`formula_version`、`generated_at`。CLI/JSON、Streamlit 和 skill 消费结果时，应能展示或透传这些信息。

所有时变数据必须按 `analysis_date` 截断，不只财务数据：
...
数据治理应作为独立 Phase 先完成，再进入 app 和 skill：

```text
Phase 6：数据治理与真实数据接入
Phase 7：Streamlit 数据工作台
Phase 8：CLI / Skill / 日报集成
```

当前 Streamlit 工作台如果已经基于 fixture/core 完成，只能视为 Phase 7 的界面验证；接入真实数据前仍必须先完成 Phase 6。Phase 7 的真实数据 DoD 是“页面读取 Phase 6 的 SQLite/repository，并展示数据日期、来源和质量 warnings”。
