...
后续接真实数据时，通过 `classification_system` 字段区分口径：
...
不要在 Phase 0-5 创建 Streamlit 页面，除非用户明确要求提前做临时可视化。
...
CLI 的 `json` 输出必须稳定。字段使用 `snake_case`。
...
在 core/CLI 稳定后做独立 Streamlit 数据工作台。
...
让 `skills/china-stock-analysis` 调用 Fundamental Screener，把基本面量化结果接入自选股和持仓场景。
