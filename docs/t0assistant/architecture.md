...
本文定义 T+0 助手 MVP 的系统边界、进程关系、数据所有权和关键运行时流程，回答“系统由哪些可运行部分组成、它们如何协作”。
...
MVP 使用一个 Electron 应用进程组和一个 Python 子进程，不拆分为远程微服务。每个 App 实例只管理自己的 Python 子进程。
...
- 具体采用 HTTP + WebSocket、HTTP + SSE 或等价实现，由后续 ADR 固化；无论传输实现如何，逻辑契约保持请求与事件分离；
...
回放 Session 是一次性的内存对象，使用历史行情和模拟时钟。进入回放只带入 App 当前股票；未选股时可以先进入空白回放模式，随后选中的股票同时成为 App 当前股票，并由 Coordinator 创建后台 Live Session 和该股票的 Replay 初始状态。开始回放时加载目标日前的 5 分钟预热数据，并按实际存在的分钟序列推进。退出回放立即销毁其日期、进度、派生画面和模拟成交，但保留已下载行情缓存。
...
管线行为必须由标准行情输入、可替换时钟和显式配置决定。同一初始条件和输入前缀必须产生相同结果。Replay 向后定位时丢弃当前管线状态，从开盘前预热状态重新创建实例并顺序重放到目标时点；后续允许使用检查点优化性能，但不得改变重建语义。MVP 不持久化或承诺兼容管线内部状态，只要求状态可丢弃、输入可重放、结果可重建。

### 7.4 并发 Session 与共享资源协调

用户进入回放时 Live Session 继续后台运行，因此 Live 与 Replay 并存是正常运行模式。两者保持 Session 和管线状态隔离，但共享进程内的行情服务、仓储基础设施和计算资源。MVP 不引入包揽所有资源的通用调度器，而是采用“两个队列、一个仓储边界”：远程行情请求和计算任务分别通过有界优先队列协调，数据库访问统一通过 Repository。
...
Session 对前端输出单调递增的修订号或等价序列标识。前端忽略已过期事件；股票切换、回放跳转和重连使用完整快照重新建立基线，避免旧请求晚到后覆盖新状态。具体字段在接口契约中定义。
...
本架构确认后，至少需要用 ADR 固化以下决策：

1. Electron 管理单个 Python 本地服务的进程模型；
2. Renderer → preload → Main → Python 的安全调用边界；
3. Python 本地请求与事件通道的具体传输方案；
4. Live/Replay 共用处理管线与可替换时钟；
5. SQLite 单写入边界、两个本地数据文件的划分、跨 App 并发访问及迁移策略；
6. 工作台完整快照、行情与指标更新、完整 CZSC 结构快照的一致性模型；
7. Provider 请求与计算任务的有界调度、Live 优先级、取消和执行模型；
8. Python 自动重启、Session 重建及纯内存 Replay 状态丢失策略；
9. Python 运行时随桌面应用打包的方式。

ADR 应记录决策理由和备选方案，不在本文件提前替代决策记录。

## 13. 架构确认清单

- [ ] Electron 主进程拥有 Python 子进程生命周期，Renderer 不直接访问 Python。
- [ ] Python 本地服务是行情、Session、计算和持久化的统一后端。
- [ ] 实盘与回放只替换输入源和时钟，共用处理管线。
- [ ] Live 与 Replay 共用管线实现但各自拥有独立实例，可变派生状态不跨 Session 共享。
- [ ] App Coordinator 是当前股票和模式的运行时权威；Session 绑定股票且不在原实例上切股。
- [ ] Live 与 Replay 共享资源时经过有界 Provider 请求队列、有界计算任务队列和 Repository 边界，Live 实时任务优先。
- [ ] Provider、分析、Repository、Session 和 Python 进程形成逐级错误边界，局部失败只停止受影响功能且不发布部分结果。
- [ ] Python 崩溃时 Electron 保持可用并有限自动重启；Live 可由持久化输入重建，纯内存 Replay 状态明确丢失。
- [ ] 外部行情、时钟、仓储和前端后端边界均可替换为确定性测试实现，核心管线可在无网络、无 Electron 环境下重放固定输入。
- [ ] 查看回放期间 Live Session 继续后台更新当前股票。
- [ ] 本地行情缓存文件保存可跨 App 复用的行情与证券主数据，本地成交与设置文件隔离真实成交、收费方案和持久化偏好。
- [ ] 回放模拟成交、指标和 CZSC 结果不持久化。
- [ ] 1 分钟 K 只形成动态 5 分钟 K；取得正式闭合 5 分钟 K 后替换动态 K，再更新正式指标和 CZSC。
- [ ] 5 分钟指标和 CZSC 基于完整已加载历史连续计算；React 只按绘图区宽度裁剪默认可见 N 根，不能用可见窗口触发重新计算。
- [ ] 5 分钟默认视口以最新或模拟时点为右边界，跨交易日满轴显示实际 K 线，并压缩所有非交易时段空槽。
- [ ] 初次加载和回放定位使用完整工作台快照；实时行情与普通指标按各自节奏更新，CZSC 重算发布完整 CZSC 结构快照。
- [ ] `apps/` 与 `packages/` 的职责符合 [`module_design.md`](./module_design.md)。
- [ ] 接口细节、ADR、技术方案和 Issue 拆分均留到架构确认之后。
