...
- 呈现三栏工作台、5 分钟主图、中间副图（分时或 30 分钟）、行情栏、回放面板和成交 Drawer；
- 保存 UI 会话状态，例如布局、图层开关、5 分钟与 30 分钟可见范围与跟随最新/手工浏览状态和当前模式；分时始终使用全日交易轴，不持久化局部时间窗口；分时/30 分钟选择不跨重启保存；
...
- Replay Session 不读取或维护成交；真实成交由独立仓储按股票和日期提供；
...
**09:30 原子切日**：09:30 前始终展示上一交易日；09:30 后收到当日有效 quote 或首根有效分钟 K 后，准备当日基线并一次性替换。产品层保持同一股票连续 Live 身份；切日使用独立的 `market_epoch` / `pipeline_epoch`，不得复用 `service_generation`（Python 进程代次）。旧 epoch 任务与快照必须拒绝；对外 revision 单调递增。

**轮询档位**（`polling_profile: active | reduced | idle`）：

| 时段 | 策略 |
| --- | --- |
| 交易日上午、下午 | Active（现有 quote/1m/5m cadence） |
| 09:30 前 | 不轮询；09:30 唤醒 |
| 午休 11:30–13:00 | 暂停；13:00 唤醒 |
| 15:00 后 | 约 15:05 收盘对账后 idle |
| 周末、节假日 | idle；下一交易日 09:30 唤醒 |
...
| 真实手工成交 | Python 成交仓储 | 本地成交与设置文件 | 仅 T+0 助手访问；按股票/日期查询；Replay 和历史日图仅展示，不提供增删改入口 |
| 收费方案 | Python 配置仓储 | 本地成交与设置文件 | 保存结构化费率；不追溯历史成交 |
| 图表实例与 5 分钟可见范围 | React 工作台状态 | 否 | 布局/模式切换期间保持；分时不持久化局部窗口 |
| 5 分钟跟随最新/手工浏览状态 | React 工作台状态 | 否 | 仅 5 分钟：决定宽度变化和增量更新时是右对齐重算 N，还是保持用户逻辑可见范围 |
| 技术日志 | Electron/Python 日志设施 | 本地文件 | Log Review 只读展示 |

行情仓储必须保留“真实零”和“未知”之间的区别。Provider 明确报告的零可以持久化为 `0`；
无法获得的量额必须持久化为 SQL `NULL`。历史缓存中由旧腾讯预热逻辑写入的
`amount = 0` 按以下确定顺序解释：`volume = 0` 时把该 bar 视为一致的真实零并保留
`amount = 0`；`volume > 0` 且没有 Provider 明确零值来源时，属于旧版未知哨兵；
`volume IS NULL` 时无法证明零成交额，也规范化为 `amount = null`。T0-009 必须执行
幂等写回迁移，把后两类旧哨兵写成 SQL `NULL` 并保留迁移计数；在迁移完成前及对只读
旧库，Repository 读取映射仍必须执行同一规范化作为兼容兜底。未来 Provider 明确报告
的零及其内部来源元数据不受该旧数据规则误伤。公共快照不新增 evidence 结构。
...
sequenceDiagram
    ...
    Data-->>Replay: 目标范围的标准价格序列（未知量额为 null）
    alt 1m 价格 K 线可用
        ...
        Replay->>Events: 发布完整工作台快照
        Events-->>UI: 转发（经 Gateway）
    else 1m 与 5m 价格 K 线均不可用
        Replay->>Events: 发布“无可用价格 K 线”
        Events-->>UI: 显示错误、重试和返回
    end
    opt volume 或 amount 不可用
        Replay->>Events: 按当前已发布前缀生成 warning
        Events-->>UI: 价格继续回放，依赖区域显示 N/A，不弹全局提示
    end
    opt 回放已经成功开始
        User->>UI: 拖动到目标时点
        UI->>Gateway: 暂停并定位
    ...
    User->>UI: 返回实盘
    UI->>Gateway: 返回实盘
    Gateway->>Coordinator: 销毁 Replay 行情会话
...
真实成交的新增、修改和删除由 Python 成交服务校验并在单个数据库事务中完成。写入成功后，后端不返回全库成交快照，而是对每个受影响的 `symbol + trade_date` 发布一条带显式 scope 的权威 `trades_changed` 事实；列表为空时也必须携带 scope。修改使成交跨股票或跨日期移动时，依次发布旧 scope 和新 scope 的最新事实，两条事件使用递增 revision。前端必须先经过全局 generation/revision 门闸，再进行 scope 匹配：不关心的 scope 不替换当前列表，但仍推进已见 revision。`trade_revision` 用于判断成交事实新旧，不要求按 scope 连续，也不得因数值跳号单独触发快照重建；传输缺口仍只由事件外层 revision 门闸处理。收费方案只用于生成当前录入的默认费用，成交保存的是用户最终确认金额。

`list_trades` 只是 scope 查询。对 `instrument_type: index` 的误调用也接受请求，并发布该 `symbol + trade_date` 的空事实，不把只读查询升级为错误。create/update 则按证券主数据的允许列表仅接受 `stock` 和 `etf`；指数或无法确认交易资格时返回 `error_code: trade_not_allowed`、`category: invalid_request`、`retryable: false`。delete 只根据 `trade_id` 处理已存事实，不因当前 Live/Replay 模式改变服务语义，以允许从专用历史管理入口清理旧错数据；Replay 界面不暴露该命令。

全历史读取是 #163 保证现有历史入口不因 scoped 事件迁移而中断的必做项，不代替 T0-043 对历史功能的所有权。App v1/v2 新增同步 `list_trade_history` 命令，使用 `session_id: null` 和 `trade_scope: real`，在 `command_response.data` 中返回同一仓储快照的 `trade_revision + trades`，不发布全历史事件。历史对话框打开时，任意真实 scoped `trades_changed` 只作为失效通知；前端合并连续通知后重拉一次全历史，不在本地拼接 scope。对话框关闭时不维护隐藏的权威列表，下次打开重新读取；晚到且 `trade_revision` 更旧的同步响应被丢弃。

Replay 对可交易标的只在目标 `symbol + trade_date` 就绪或改变时读取一次全天成交；播放、单步和 seek 只在 Renderer 本地重新应用游标过滤，不重新查询。仅当当前 scope 收到 `trades_changed`、服务 generation 改变或重连时重读；指数 Replay 不查询成交。
...
- Replay 补数失败时优先使用已有缓存；只有 5 分钟价格 K 线时进入 5 分钟降级回放，1 分钟和 5 分钟价格 K 线都不足时只终止本次 Replay 加载，不影响 Live；
- Replay 价格可靠性使用 Market Data Service 的范围覆盖和 `identify_missing_ranges` 结果，不使用允许缺失根数或比例阈值；已返回的任意非法价格 bar 使该粒度整体无效，不得静默丢弃，1m 无效时仍可尝试正式 5m；
- 只要任一尝试粒度已取得非法价格 bar 且最终无合法降级输入，即使另一粒度只是无数据或覆盖不足，也返回 `replay_data_invalid`；只有所有失败粒度均未发现非法 bar、只是没有价格数据或无法证明可靠覆盖时，才返回 `replay_price_data_unavailable`；新实现不得发出旧码 `replay_data_unavailable`；
- 时间戳与 OHLC 是 K 线推进和 Replay 就绪的最低门槛。`volume` 或 `amount` 缺失时标准化层保留字段为 `null`，Live/Replay 价格管线继续推进；只有直接依赖缺失字段的 VOL、VOL MA、VWAP 和成交量额展示降级为不可用；
- 未知量额不得填成 `0`，不得通过 OHLCV 估算，也不得使用未来时点或收盘快照补齐。
- Live 与 Replay 对各自当前已发布的数据前缀生成同结构的量额 warning；公共快照以 bar 的 `null` 和 warning 表达可用性，来源与质量证据只保留在内部 `ProviderIssue`、仓储元数据和日志中。
- 动态 5 分钟 K、动态日 K、由分钟形成的 quote 累计量额使用严格未知传播；任一组成 bar 的对应字段未知时聚合结果为 `null`。VOL MA 不跳过未知值；VWAP 一旦在当日累计前缀遇到未知依赖，后续保持 `null`，直至补齐后从开盘重算。
- Provider 权威 Quote 与分钟派生累计值保留独立 lineage。证券、交易日和行情时间匹配当前快照时，行情栏优先采用权威 Quote 的非空量额；这不回填分钟 bar，也不消除分钟粒度 warning。正式 5 分钟 K 替换动态 K 后，5 分钟派生指标和 warning 基于正式序列立即重算，1 分钟 warning 仍按 1 分钟前缀独立保留。
...
- Replay Session 失败时销毁其管线和进度，Live Session 与独立真实成交仓储继续可用；用户可以返回实盘或重新创建 Replay；
...
- Replay 进度是纯内存状态，Python 崩溃后明确丢失，不做静默伪恢复；真实成交从 SQLite 恢复；
- 连续自动重启失败后停止重启循环，保留日志查看和手动重试或重启 App 的入口。具体次数和退避参数由技术方案确定。

### 10.6 错误传递

跨进程只传递稳定的应用错误分类、影响范围、严重程度、是否可重试、受影响能力和关联标识；异常栈、上游原始响应和内部对象只写入本地技术日志。稳定错误必须准确对应发现它的边界：Provider/数据完整性、计算、Session、Repository 和 Python 进程生命周期分别分类，不允许用 `service_unavailable` 兜底掩盖数据或计算错误。“本地服务尚未就绪”只描述 Python 进程未启动、退出、正在重启或本地传输确实不可达。后台刷新失败使用非阻塞状态反馈，用户主动操作失败必须立即明确反馈；重试动作必须重新执行实际失败阶段，而不是无条件重连服务。具体错误码与字段由接口契约定义。
...
- [x] Replay Session 不拥有成交状态；真实成交独立持久化，指标和 CZSC 结果不持久化。
...
- [x] 分时默认视口固定为目标日完整交易分钟轴（`09:30→15:00`），已发生行情从左向右填充；不与 5 分钟共用右对齐跟随。
