...
| Replay 基线 | [`replay_interface_and_behavior.md`](./replay_interface_and_behavior.md) v0.9；T0-056 补充播放速率后升为 v1.0 |
...
本文不重新讨论已经冻结的产品、架构、模块和 Replay v0.9 契约。实施中如发现必须
改变公共契约、目录所有权或 PRD 边界，应单独提出变更 Issue，不能在普通功能 PR 中
顺手修改。Replay v0.9 未定义 PRD 已要求的 `1×/2×/5×/10×` 播放速率命令和状态字段；
这是当前唯一已知的 Replay 未定义项，必须通过 T0-056 补充并冻结为 v1.0 接受基线，不能由
T0-046 或 T0-049 自行扩展。
...
| FR-07：单日历史回放 | Replay v0.9 主契约已冻结；播放速率契约待 T0-056 补充并升为 v1.0 | 未实现 | Epic G、H |
...
| T0-003 | 定义 Live、成交和偏好命令/事件契约 | 集成负责人 | FR-03/04/05 | T0-002 | 只补 Live/成交/偏好；直接引用 Replay v0.9，不重定义或降级 Replay 字段 |
| T0-004 | 建立跨 Python/TypeScript 契约 Fixture、Fake Safe Bridge 和兼容性测试 | 集成负责人 | 全部 | T0-002, T0-003 | Python/TS 可消费同一 fixture；包含完整快照、增量、乱序和错误；Replay 示例使用合并时最新的接受版本：若 T0-056 已合并则直接使用 v1.0，否则先使用 v0.9 并由 T0-056 后续统一升级 |
...
| ID | Issue | 主责/目录 | 覆盖 | 依赖 | 交付与验收 |
| --- | --- | --- | --- | --- | --- |
| T0-056 | 补充并冻结 Replay 播放速率契约 | 集成负责人 / Replay 契约与固定测试数据 | FR-07 | — | 直接以 PRD 和 Replay v0.9 为输入，定义四档播放速率命令、权威状态、revision 和幂等/错误语义；把 Replay 契约版本升为 v1.0，并更新跨语言固定测试数据及所有版本引用；不实现播放逻辑或 UI |
...
| T0-002 | 只依赖已冻结的 PRD、架构、模块和 Replay v0.9 文档 |
| T0-056 | 直接以冻结的 PRD 和 Replay v0.9 为输入，可独立补充播放速率契约 |
...
1. Replay v0.9 是冻结开发输入，但其未覆盖的播放速率契约只能由 T0-056 补充，并把
   契约版本升为 v1.0；普通 Replay 实现 Issue 不得修改 v0.9 或接受后的 v1.0 契约。
...
- [ ] 所有执行者已阅读 PRD、架构、模块设计、UI 规格、Replay v0.9、T0-056 形成的 Replay v1.0 和相关 ADR。
