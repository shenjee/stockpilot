...
- 验收文件提交 SHA：`d3de2344ab6d14103ff271bc48e2b01f61dc59a1`（验收脚本/产物/本报告首次入库）
- UI 冒烟证据提交 SHA：`e8e2a3116daaa6ce4c1313e8b7bd141f30dc7316`（Live/Replay/chan-viewer 截图与步骤记录；不自指本行回填提交）
- 父 issue：#172；本报告对应子任务 #176
- **门禁（本轮更新）**：#174/#175 已在升级分支完成正式验收记录（`acceptance-report-174.md` / `acceptance-report-175.md`）。不以 issue CLOSED 代替验收。已确认的行为差异与性能结论不再询问。
- 正式短样本 fixture 已按真实输出修正（见 §9）；长冻结基线仍承担结构金标。
- 本次 **不** 代表 #177 正式切换完成，也 **不** 代表 #172 可关闭。合入后运行验证仍待补。
...
## 9. 正式 fixture（本轮已更新；验收归属 #176）

#174/#175 升级分支验收通过后，按真实 `analyze()` 输出修正短样本：

| 文件 | 变更原因 |
|---|---|
| `packages/chantheory/tests/fixtures/p2_sample_result.json` | 旧内容手写了 4 分型/3 笔，与 0.10.12 和 1.0.1 真实输出都不符。现改为默认 `analyze()` 在 5 根日线上的真实结果：0 分型 / 0 笔，`INSUFFICIENT_BARS`，`engine_version` 与声明 pin 一致。 |
| `test_fixture_engine_version_matches_pin` | 改为读取 `PINNED_ENGINE_VERSION`，不再硬编码版本号。 |
| `test_short_p2_sample_is_boundary_and_matches_real_engine` | 锁定短样本的边界用途：不足 K 线、无结构。 |

原则：

1. 冻结长样本（5m/30m/日线）继续承担结构金标；旧版 `baseline/` 永久保留 0.10.12 语义。
2. 5 根短样本只作边界测试，不当引擎金标。
3. 下游/投影测试里作为**历史分析结果**出现的 `engine_version: 0.10.12` mock **不改**。
...
| #174/#175 正式验收 | **升级分支通过**（见对应验收记录） |
| 正式短样本 fixture | **已按真实输出修正** |
| #176 | 升级分支验收可关闭；不以 CLOSED 代替判断 |
| #177 合入前准备 | 切换方案/依赖快照/回退演练见 `switch/` |
| #177 / #172 完成 | **否** — 合入后正式运行验证未做 |
